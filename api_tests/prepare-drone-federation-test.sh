...
if [ ! -f "$PICTRS_PATH" ]; then
  ...
  while [ ! -f "$PICTRS_PATH" ] && [ "$count" -lt 3 ]; do
    # This one sometimes goes down, and is blocked by Anubis
    #curl "https://git.asonix.dog/asonix/pict-rs/releases/download/v0.5.5/pict-rs-linux-amd64" -o "$PICTRS_PATH"
