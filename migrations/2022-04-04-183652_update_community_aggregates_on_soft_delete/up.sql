...
ELSIF (TG_OP = 'DELETE'  OR (TG_OP = 'UPDATE' AND OLD.deleted = 'f' AND NEW.deleted = 't')) THEN
update community_aggregates
...
set posts = coalesce(cd.posts, 0),
    ...
    from ( 
      ...
      from community c
      left join post p on c.id = p.community_id and p.deleted = 'f'
      left join comment ct on p.id = ct.post_id and ct.deleted = 'f'
      group by c.id
    ) cd
