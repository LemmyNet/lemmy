...
pub async fn setup_federation_scheduled_tasks(
  ...
  scheduler.every(TimeUnits::minutes(1)).run(|| async {
    let mut conn =
      PgConnection::establish(DB_URL.get().unwrap()).expect("could not establish connection");
  ...
  scheduler.every(TimeUnits::hour(1)).run(|| async {
    let mut conn =
      PgConnection::establish(DB_URL.get().unwrap()).expect("could not establish connection");
    update_dead_instances(&mut conn).await;
  });
...
async fn check_dead_instances(conn: &mut PgConnection, context: &LemmyContext) -> LemmyResult<()> {
  ...
  let now = naive_now();
  let instances: Vec<Instance> = Instance::read_all(context.pool())
    .await?
    .into_iter()
  ...
  for instance in instances {
    ...
    let res = context.client().get(&node_info_url).send().await;
