...
async fn apub_columns_2021_02_02(mut pool: &mut DbPool<'_>) -> Result<(), LemmyError> {
  let conn = &mut get_conn(pool).await?;
...
async fn regenerate_public_keys_2022_07_05(mut pool: &mut DbPool<'_>) -> Result<(), LemmyError> {
  let conn = &mut get_conn(pool).await?;
