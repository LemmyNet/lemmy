...
impl CommunityQuery<'_> {
  pub async fn list(
    ...
    let mut pq =
      CommunityView::paginate(query, o.page_cursor.clone(), sort_direction, pool, None).await?;
...
impl MultiCommunityQuery {
  pub async fn list(self, pool: &mut DbPool<'_>) -> LemmyResult<PagedResponse<MultiCommunityView>> {
    ...
    let mut pq =
      MultiCommunityView::paginate(query, o.page_cursor.clone(), sort_direction, pool, None)
        .await?;
