...
impl LocalUserView {
  pub async fn read(mut pool: DbPool<'_>, local_user_id: LocalUserId) -> Result<Self, Error> {
  ...
  pub async fn read_person(mut pool: DbPool<'_>, person_id: PersonId) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
    let (local_user, person, counts) = local_user::table
  ...
  pub async fn read_from_name(mut pool: DbPool<'_>, name: &str) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
    let (local_user, person, counts) = local_user::table
  ...
  pub async fn find_by_email(mut pool: DbPool<'_>, from_email: &str) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
    let (local_user, person, counts) = local_user::table
      .inner_join(person::table)
      .inner_join(person_aggregates::table.on(person::id.eq(person_aggregates::person_id)))
  ...
  pub async fn list_admins_with_emails(mut pool: DbPool<'_>) -> Result<Vec<Self>, Error> {
    let conn = &mut get_conn(pool).await?;
