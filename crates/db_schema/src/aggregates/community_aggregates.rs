...
impl CommunityAggregates {
  pub async fn read(conn: &mut DbConn, community_id: CommunityId) -> Result<Self, Error> {
...
mod tests {
  ...
  async fn test_crud() {
    ...
    let another_inserted_community = Community::create(conn, &another_community).await.unwrap();
    ...
    let community_aggregates_before_delete = CommunityAggregates::read(conn, inserted_community.id)
      .await
      .unwrap();
    ...
    let another_community_aggs = CommunityAggregates::read(conn, another_inserted_community.id)
      .await
      .unwrap();
    ...
    let another_community_num_deleted = Community::delete(conn, another_inserted_community.id)
      .await
      .unwrap();
