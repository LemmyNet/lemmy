...
impl Crud for Person {
  ...
  async fn read(pool: DbPool<'_>, person_id: PersonId) -> Result<Self, Error> {
  ...
  async fn delete(pool: DbPool<'_>, person_id: PersonId) -> Result<usize, Error> {
    let conn = &mut get_conn(pool).await?;
    diesel::delete(person::table.find(person_id))
      .execute(conn)
      .await
  }
  async fn create(pool: DbPool<'_>, form: &PersonInsertForm) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
...
impl Person {
  ...
  pub async fn upsert(pool: DbPool<'_>, form: &PersonInsertForm) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
    insert_into(person::table)
      .values(form)
  ...
  pub async fn delete_account(pool: DbPool<'_>, person_id: PersonId) -> Result<Person, Error> {
    let conn = &mut get_conn(pool).await?;
...
impl Followable for PersonFollower {
  ...
  async fn follow(pool: DbPool<'_>, form: &PersonFollowerForm) -> Result<Self, Error> {
  ...
  async fn follow_accepted(_: DbPool<'_>, _: CommunityId, _: PersonId) -> Result<Self, Error> {
    unimplemented!()
  }
  async fn unfollow(pool: DbPool<'_>, form: &PersonFollowerForm) -> Result<usize, Error> {
    use crate::schema::person_follower::dsl::{follower_id, person_follower, person_id};
    let conn = &mut get_conn(pool).await?;
    diesel::delete(
      person_follower
