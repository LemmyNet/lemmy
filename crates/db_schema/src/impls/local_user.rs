...
impl LocalUser {
  ...
  pub async fn set_all_users_email_verified(pool: DbPoolRef<'_>) -> Result<Vec<Self>, Error> {
    let conn = pool;
  ...
  pub async fn is_email_taken(pool: DbPoolRef<'_>, email_: &str) -> Result<bool, Error> {
...
impl Crud for LocalUser {
  ...
  async fn read(pool: DbPoolRef<'_>, local_user_id: LocalUserId) -> Result<Self, Error> {
    let conn = pool;
    local_user.find(local_user_id).first::<Self>(conn).await
  }
  async fn delete(pool: DbPoolRef<'_>, local_user_id: LocalUserId) -> Result<usize, Error> {
    let conn = pool;
