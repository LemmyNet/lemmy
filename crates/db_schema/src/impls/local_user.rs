...
impl LocalUser {
  ...
  pub async fn set_all_users_email_verified(pool: DbPool<'_>) -> Result<Vec<Self>, Error> {
    let conn = &mut get_conn(pool).await?;
  ...
  pub async fn is_email_taken(pool: DbPool<'_>, email_: &str) -> Result<bool, Error> {
...
impl Crud for LocalUser {
  ...
  async fn read(pool: DbPool<'_>, local_user_id: LocalUserId) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
    local_user.find(local_user_id).first::<Self>(conn).await
  }
  async fn delete(pool: DbPool<'_>, local_user_id: LocalUserId) -> Result<usize, Error> {
    let conn = &mut get_conn(pool).await?;
  ...
  async fn create(mut pool: DbPool<'_>, form: &Self::InsertForm) -> Result<Self, Error> {
    ...
    let site_languages = SiteLanguage::read_local_raw(pool.clone()).await;
    if let Ok(langs) = site_languages {
      // if site exists, init user with site languages
      LocalUserLanguage::update(pool.clone(), langs, local_user_.id).await?;
    } else {
