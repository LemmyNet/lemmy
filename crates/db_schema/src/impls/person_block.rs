...
impl Blockable for PersonBlock {
  ...
  async fn block(pool: DbPool<'_>, person_block_form: &PersonBlockForm) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
  ...
  async fn unblock(pool: DbPool<'_>, person_block_form: &Self::Form) -> Result<usize, Error> {
    let conn = &mut get_conn(pool).await?;
    diesel::delete(
      person_block
