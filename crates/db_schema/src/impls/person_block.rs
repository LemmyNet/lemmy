...
impl Blockable for PersonBlock {
  ...
  async fn block(pool: DbPoolRef<'_>, person_block_form: &PersonBlockForm) -> Result<Self, Error> {
    let conn = pool;
  ...
  async fn unblock(pool: DbPoolRef<'_>, person_block_form: &Self::Form) -> Result<usize, Error> {
    let conn = pool;
    diesel::delete(
      person_block
