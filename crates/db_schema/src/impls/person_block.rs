...
impl Blockable for PersonBlock {
  ...
  async fn block(conn: &mut DbConn, person_block_form: &PersonBlockForm) -> Result<Self, Error> {
  ...
  async fn unblock(conn: &mut DbConn, person_block_form: &Self::Form) -> Result<usize, Error> {
    diesel::delete(
      person_block
