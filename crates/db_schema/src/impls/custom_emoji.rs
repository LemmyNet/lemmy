...
impl CustomEmoji {
  pub async fn create(pool: DbPool<'_>, form: &CustomEmojiInsertForm) -> Result<Self, Error> {
  ...
  pub async fn delete(pool: DbPool<'_>, emoji_id: CustomEmojiId) -> Result<usize, Error> {
    let conn = &mut get_conn(pool).await?;
