...
mod tests {
  ...
  fn test_crud() {
    ...
    let inserted_activity = Activity::create(&mut conn, &activity_form).unwrap();
    ...
    let read_activity = Activity::read(&mut conn, inserted_activity.id).unwrap();
    let read_activity_by_apub_id = Activity::read_from_apub_id(&mut conn, &ap_id).unwrap();
    Person::delete(&mut conn, inserted_creator.id).unwrap();
    Activity::delete(&mut conn, inserted_activity.id).unwrap();
