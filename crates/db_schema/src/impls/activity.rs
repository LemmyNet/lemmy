...
impl Crud for Activity {
  ...
  type IdType = i32;
  async fn read(pool: DbPool<'_>, activity_id: i32) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
    activity.find(activity_id).first::<Self>(conn).await
  }

  async fn create(pool: DbPool<'_>, new_activity: &Self::InsertForm) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
  ...
  async fn delete(pool: DbPool<'_>, activity_id: i32) -> Result<usize, Error> {
    let conn = &mut get_conn(pool).await?;
