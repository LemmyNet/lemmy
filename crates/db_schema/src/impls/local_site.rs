...
impl LocalSite {
  pub async fn create(pool: DbPool<'_>, form: &LocalSiteInsertForm) -> Result<Self, Error> {
  ...
  pub async fn update(pool: DbPool<'_>, form: &LocalSiteUpdateForm) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
  ...
  pub async fn delete(pool: DbPool<'_>) -> Result<usize, Error> {
    let conn = &mut get_conn(pool).await?;
