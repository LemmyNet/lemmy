...
impl Crud for Community {
  ...
  async fn read(pool: DbPool<'_>, community_id: CommunityId) -> Result<Self, Error> {
  ...
  async fn delete(pool: DbPool<'_>, community_id: CommunityId) -> Result<usize, Error> {
    let conn = &mut get_conn(pool).await?;
  ...
  async fn create(mut pool: DbPool<'_>, form: &Self::InsertForm) -> Result<Self, Error> {
    let is_new_community = match &form.actor_id {
      Some(id) => Community::read_from_apub_id(pool.clone(), id)
        .await?
        .is_none(),
      None => true,
    };
...
impl Followable for CommunityFollower {
  ...
  async fn follow(pool: DbPool<'_>, form: &CommunityFollowerForm) -> Result<Self, Error> {
  ...
  async fn unfollow(pool: DbPool<'_>, form: &CommunityFollowerForm) -> Result<usize, Error> {
    use crate::schema::community_follower::dsl::{community_follower, community_id, person_id};
    let conn = &mut get_conn(pool).await?;
    diesel::delete(
      community_follower
