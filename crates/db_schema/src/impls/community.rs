...
impl Joinable for CommunityActions {
  ...
  async fn leave(pool: &mut DbPool<'_>, form: &Self::Form) -> LemmyResult<usize> {
    let conn = &mut get_conn(pool).await?;
    diesel::update(community_actions::table.find((form.person_id, form.community_id)))
      .set(community_actions::became_moderator_at.eq(None::<DateTime<Utc>>))
      .execute(conn)
      .await
      .with_lemmy_type(LemmyErrorType::CommunityModeratorAlreadyExists)
  }
...
impl Followable for CommunityActions {
  ...
  async fn unfollow(
    ...
    let conn = &mut get_conn(pool).await?;
    diesel::update(community_actions::table.find((person_id, community_id)))
      .set((
        community_actions::followed_at.eq(None::<DateTime<Utc>>),
        community_actions::follow_state.eq(None::<CommunityFollowerState>),
        community_actions::follow_approver_id.eq(None::<PersonId>),
      ))
      .execute(conn)
      .await
      .with_lemmy_type(LemmyErrorType::CommunityFollowerAlreadyExists)
...
impl Blockable for CommunityActions {
  ...
  async fn unblock(pool: &mut DbPool<'_>, community_block_form: &Self::Form) -> LemmyResult<usize> {
    let conn = &mut get_conn(pool).await?;
    diesel::update(community_actions::table.find((
      community_block_form.person_id,
      community_block_form.community_id,
    )))
    .set(community_actions::blocked_at.eq(None::<DateTime<Utc>>))
    .execute(conn)
    .await
    .with_lemmy_type(LemmyErrorType::CommunityBlockAlreadyExists)
  }
