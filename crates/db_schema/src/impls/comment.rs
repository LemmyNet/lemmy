...
impl Crud for Comment {
  ...
  async fn read(pool: DbPool<'_>, comment_id: CommentId) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
    comment.find(comment_id).first::<Self>(conn).await
  }

  async fn delete(pool: DbPool<'_>, comment_id: CommentId) -> Result<usize, Error> {
    let conn = &mut get_conn(pool).await?;
    diesel::delete(comment.find(comment_id)).execute(conn).await
  }

  /// This is unimplemented, use [[Comment::create]]
  async fn create(_pool: DbPool<'_>, _comment_form: &Self::InsertForm) -> Result<Self, Error> {
    unimplemented!();
  }
...
impl Likeable for CommentLike {
  ...
  type IdType = CommentId;
  async fn like(pool: DbPool<'_>, comment_like_form: &CommentLikeForm) -> Result<Self, Error> {
