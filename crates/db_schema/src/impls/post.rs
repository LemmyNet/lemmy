...
impl Crud for Post {
  ...
  type IdType = PostId;
  async fn read(pool: DbPool<'_>, post_id: PostId) -> Result<Self, Error> {
    let conn = &mut get_conn(pool).await?;
    post.find(post_id).first::<Self>(conn).await
  }

  async fn delete(pool: DbPool<'_>, post_id: PostId) -> Result<usize, Error> {
    let conn = &mut get_conn(pool).await?;
...
impl Likeable for PostLike {
  ...
  type IdType = PostId;
  async fn like(pool: DbPool<'_>, post_like_form: &PostLikeForm) -> Result<Self, Error> {
...
impl Saveable for PostSaved {
  ...
  async fn save(pool: DbPool<'_>, post_saved_form: &PostSavedForm) -> Result<Self, Error> {
  ...
  async fn unsave(pool: DbPool<'_>, post_saved_form: &PostSavedForm) -> Result<usize, Error> {
    use crate::schema::post_saved::dsl::{person_id, post_id, post_saved};
    let conn = &mut get_conn(pool).await?;
    diesel::delete(
      post_saved
