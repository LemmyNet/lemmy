...
mod tests {
  ...
  fn test_crud() {
    ...
    let inserted_post_saved = PostSaved::save(&mut conn, &post_saved_form).unwrap();
    ...
    let inserted_post_read = PostRead::mark_as_read(&mut conn, &post_read_form).unwrap();
    ...
    let read_post = Post::read(&mut conn, inserted_post.id).unwrap();
    let updated_post = Post::update(&mut conn, inserted_post.id, &new_post).unwrap();
    let like_removed = PostLike::remove(&mut conn, inserted_person.id, inserted_post.id).unwrap();
    let saved_removed = PostSaved::unsave(&mut conn, &post_saved_form).unwrap();
    let read_removed = PostRead::mark_as_unread(&mut conn, &post_read_form).unwrap();
