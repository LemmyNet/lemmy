use crate::{
  newtypes::LocalUserId, schema::password_reset_request::dsl::*, source::password_reset_request::*,
  traits::Crud,
};
...
mod tests {
  ...
  fn test_crud() {
    ...
    let inserted_local_user = LocalUser::create(&mut conn, &new_local_user).unwrap();

    let token = "nope";
    let token_encrypted_ = "ca3704aa0b06f5954c79ee837faa152d84d6b2d42838f0637a15eda8337dbdce";

    let inserted_password_reset_request =
      PasswordResetRequest::create_token(&mut conn, inserted_local_user.id, token).unwrap();
    ...
    let read_password_reset_request =
      PasswordResetRequest::read_from_token(&mut conn, token).unwrap();
