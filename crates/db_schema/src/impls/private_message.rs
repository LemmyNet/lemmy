...
mod tests {
  ...
  fn test_crud() {
    ...
    let inserted_creator = Person::create(&mut conn, &creator_form).unwrap();
    ...
    let inserted_private_message = PrivateMessage::create(&mut conn, &private_message_form).unwrap();
    ...
    let read_private_message = PrivateMessage::read(&mut conn, inserted_private_message.id).unwrap();
    let updated_private_message =
      PrivateMessage::update(&mut conn, inserted_private_message.id, &private_message_form).unwrap();
    let deleted_private_message =
      PrivateMessage::update_deleted(&mut conn, inserted_private_message.id, true).unwrap();
    let marked_read_private_message =
      PrivateMessage::update_read(&mut conn, inserted_private_message.id, true).unwrap();
    Person::delete(&mut conn, inserted_creator.id).unwrap();
