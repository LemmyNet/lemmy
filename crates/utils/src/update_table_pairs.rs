...
fn main() -> std::io::Result<()> {
  let mut result = "// @generated by scripts/lint.sh\nuse crate::schema::*;use diesel::query_source::TableNotEqual;
  trait HasTupleId:diesel::query_source::Table{type TupleId;}struct IsThisOne;struct IsNotThisOne;
  trait NotEqTup<T> {}
  impl<X> NotEqTup<(X,)> for () {}
  impl<X> NotEqTup<()> for (X,) {}
  impl<X, Y> NotEqTup<(X,)> for (Y,) where X: NotEqTup<Y> {}".to_owned();
  ...
  for (i, table) in tables.iter().enumerate() {
    let mut tuple_inner = String::new();
    for _ in 0..i {
      tuple_inner+="(";
    }
    for _ in 0..i {
    ...
    result+="impl HasTupleId for ";
    result+=table;
    result+="::table{type TupleId=(";
    result+= tuple_inner.as_str();
    result+=");}impl<T: HasTupleId> TableNotEqual<T> for ";
    result+=table;
    result+="::table where T::TupleId: NotEqTup<(";
    result+= tuple_inner.as_str();
    result+=")> {}";
    /*write!(
      &mut result,
