...
use lemmy_db_schema::{
    ...
    utils::{Queries, ReadFn, ListFn, DbConn, DbPool, limit_and_offset, get_conn}, SortType,
};
...
> {
    let all_joins = |query: community_follower::BoxedQuery<'a, Pg>, my_person_id: Option<PersonId>| {
        let person_id_join = my_person_id.unwrap_or(PersonId(-1));

        query
            .inner_join(community::table.on(
            ...
            .left_join(post_read::table.on(
                    post_read::post_id.eq(post::id)
    ...
    let list =
        move |mut conn: DbConn<'a>,
        options: FollowedCommunityPostQuery| async move {
