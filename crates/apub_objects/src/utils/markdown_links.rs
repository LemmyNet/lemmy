...
pub(crate) async fn to_local_url(url: &str, context: &Data<LemmyContext>) -> Option<Url> {
  ...
  match dereferenced {
    Left(x) => match x {
      Left(x) => match x {
        Left(post) => post.local_url(context.settings()),
        Right(comment) => comment.local_url(context.settings()),
      }
      .ok()
      .map(Into::into),
      Right(x) => match x {
        Left(user) => format_actor_url(&user.name, "u", user.instance_id, context).await,
        Right(community) => {
