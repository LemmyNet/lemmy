...
use lemmy_api_common::{
    ...
    utils::{
        check_community_user_action, check_post_deleted_or_removed, get_url_blocklist,
        is_mod_or_admin, local_site_to_slur_regex, process_markdown, update_read_comments,
    },
};
use lemmy_db_schema::{
...
pub async fn create_comment(
    ...
    let is_mod_or_admin =
        is_mod_or_admin(&mut context.pool(), &local_user_view.person, community_id)
            .await
            .is_ok();
    ...
    if let Some(parent) = parent_opt {
        ...
        let comment_reply =
            CommentReply::read_by_comment_and_person(&mut context.pool(), parent_id, person_id)
                .await;
