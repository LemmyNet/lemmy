...
async fn active_counts(pool: &mut DbPool<'_>) -> LemmyResult<()> {
  ...
  for (full_form, abbr) in &intervals {
    ...
    let update_community_stmt = format!("update community ca set non_0_users_active_{} = nullif(mv.count_, 0) from r.community_aggregates_activity('{}') mv where ca.id = mv.community_id_", abbr, full_form);
    sql_query(update_community_stmt).execute(conn).await?;
  }

  // TODO: only run the function once
  let update_interactions_stmt = "update community ca set non_0_interactions_month = nullif(mv.count_, 0) from r.community_aggregates_interactions('1 month') mv where ca.id = mv.community_id_";
  sql_query(update_interactions_stmt).execute(conn).await?;
  let post_update_interactions_stmt = "update post pa set non_0_community_interactions_month = nullif(mv.count_, 0) from r.community_aggregates_interactions('1 month') mv where pa.community_id = mv.community_id_";
  sql_query(post_update_interactions_stmt)
    .execute(conn)
    .await?;
