...
async fn user_updates_2020_04_02(pool: &mut DbPool<'_>, settings: &Settings) -> LemmyResult<()> {
  use lemmy_db_lemmy_db_schema_file::lemmy_db_schema_file::person::dsl::{ap_id, local, person};
...
async fn community_updates_2020_04_02(
  ...
  use lemmy_db_lemmy_db_schema_file::lemmy_db_schema_file::community::dsl::{ap_id, community, local};
  let conn = &mut get_conn(pool).await?;
...
async fn post_updates_2020_04_03(pool: &mut DbPool<'_>, settings: &Settings) -> LemmyResult<()> {
  use lemmy_db_lemmy_db_schema_file::lemmy_db_schema_file::post::dsl::{ap_id, local, post};
  let conn = &mut get_conn(pool).await?;
...
async fn comment_updates_2020_04_03(pool: &mut DbPool<'_>, settings: &Settings) -> LemmyResult<()> {
  use lemmy_db_lemmy_db_schema_file::lemmy_db_schema_file::comment::dsl::{ap_id, comment, local};
  let conn = &mut get_conn(pool).await?;
...
async fn private_message_updates_2020_05_05(
  ...
  use lemmy_db_lemmy_db_schema_file::lemmy_db_schema_file::private_message::dsl::{ap_id, local, private_message};
  let conn = &mut get_conn(pool).await?;
...
async fn post_thumbnail_url_updates_2020_07_27(
  ...
  use lemmy_db_lemmy_db_schema_file::lemmy_db_schema_file::post::dsl::{post, thumbnail_url};
  let conn = &mut get_conn(pool).await?;
...
async fn apub_columns_2021_02_02(pool: &mut DbPool<'_>) -> LemmyResult<()> {
  ...
  {
    use lemmy_db_lemmy_db_schema_file::lemmy_db_schema_file::person::dsl::{inbox_url, person};
    let persons = person
  ...
  {
    use lemmy_db_lemmy_db_schema_file::lemmy_db_schema_file::community::dsl::{community, followers_url, inbox_url};
...
async fn regenerate_public_keys_2022_07_05(pool: &mut DbPool<'_>) -> LemmyResult<()> {
...

  {
    ...
    use lemmy_db_lemmy_db_schema_file::lemmy_db_schema_file::community::dsl::{community, local, public_key};
...

  {
    ...
    use lemmy_db_lemmy_db_schema_file::lemmy_db_schema_file::person::dsl::{local, person, public_key};
    let persons = person
      .filter(local.eq(true))
      .filter(public_key.eq(""))
      .load::<Person>(conn)
