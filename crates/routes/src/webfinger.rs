...
async fn get_webfinger_response(
  ...
  let user_id: Option<Url> = Person::read_from_name(context.conn().await?, &name_, false)
    .await
    .ok()
    .map(|c| c.actor_id.into());
  let community_id: Option<Url> = Community::read_from_name(context.conn().await?, &name, false)
    .await
    .ok()
    .map(|c| c.actor_id.into());
