  ...
  DbActor: ApubActor + Send + 'static,
{
  ...
  if identifier.contains('@') {
    ...
    let actor = DbActor::read_from_name_and_domain(&mut *context.conn().await?, name, domain).await;
    if actor.is_ok() {
      Ok(actor?.into())
  ...
  // local actor
    ...
    Ok(
      DbActor::read_from_name(&mut *context.conn().await?, &identifier, include_deleted)
        .await?
        .into(),
    )
  }
