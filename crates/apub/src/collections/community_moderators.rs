...
impl Collection for ApubCommunityModerators {
  ...
  async fn read_local(
    ...
    let moderators = CommunityModeratorView::for_community(data.conn().await?, owner.id).await?;
  ...
  async fn from_json(
    ...
    let current_moderators =
      CommunityModeratorView::for_community(data.conn().await?, community_id).await?;
    ...
    for mod_user in &current_moderators {
      ...
      if !apub.ordered_items.contains(&mod_id) {
        ...
        CommunityModerator::leave(data.conn().await?, &community_moderator_form).await?;
      }
    }
    ...
    for mod_id in apub.ordered_items {
        ...
        .any(|x| x == mod_user.actor_id)
      {
        ...
        CommunityModerator::join(data.conn().await?, &community_moderator_form).await?;
      }
    }
